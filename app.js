...
function displaySections(curso) {
    ...
    curso.secciones.forEach(seccion => {
        ...
        const profTexto = seccion.profesor || 'Por confirmar';
        summarySpan.innerHTML = `<span class="profesor-label">👤 ${profTexto}</span><span class="horario-label">${formatScheduleSummary(seccion.horario)}</span>`;
    ...
    horarioSeleccionado.forEach(curso => {
        curso.horario.forEach(bloque => {
            ...
            if (cell && !cell.classList.contains('lunch-break')) {
                ...
                const profAbrev = curso.profesor ? curso.profesor.split(' ').slice(-2).join(' ') : '';
                blockDiv.innerHTML = `
                    <span class="block-sigla">${curso.sigla}-${curso.seccionId}</span>
                    <span class="block-tipo">${bloque.tipo}</span>
                    ${profAbrev ? `<span class="block-prof">${profAbrev}</span>` : ''}
                `;
...
function renderSelectedList() {
    ...
    table.innerHTML = `<thead><tr><th>Código</th><th>Sec.</th><th>Nombre</th><th>Profesor</th><th></th></tr></thead><tbody></tbody>`;
    ...
    horarioSeleccionado.forEach(curso => {
        ...
        tr.innerHTML = `
            ...
            <td class="text-truncate profesor-cell" title="${curso.profesor || ''}">${curso.profesor || 'Por confirmar'}</td>
